...
function DatasetView() {
  ...
  useEffect(() => {
    ...
    void Promise.all([getArtists(dataset), getUmapProjection(dataset)])
      .then(([a, c]) => {
        setArtists(a);
        setCoords(c);
        // Clear artist from URL if not in new dataset
        if (artistId && !a.some(artist => artist.id === artistId)) {
          void navigate({ to: "/$dataset", params: { dataset } });
        }
      })
      .catch((e: Error) => setError(e.message))
  ...
  const handleDatasetChange = (newDataset: string | null) => {
    ...
    if (artistId) {
      void navigate({ to: "/$dataset/$artistId", params: { dataset: newDataset, artistId } });
    }
    else {
      void navigate({ to: "/$dataset", params: { dataset: newDataset } });
    }
  };

  const handleSelectArtist = (artist: Artist | null) => {
    if (artist) {
      void navigate({ to: "/$dataset/$artistId", params: { dataset, artistId: artist.id } });
    }
    else {
      void navigate({ to: "/$dataset", params: { dataset } });
    }
